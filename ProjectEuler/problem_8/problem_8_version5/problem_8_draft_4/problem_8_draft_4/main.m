...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSMutableArray *numberArray3 = [[NSMutableArray alloc]init];
        ...
        for (int x = 0; x < bignumber.length; x ++) {
            ...
            [numberArray3 addObject:numberForArray];
       ...
       NSArray *arrayDecreased = [arrayDecreaseInstance decreaseArray:numberArray];
        ...
        //3rd array for other attempt
        
        arraySorter *newTry = [[arraySorter alloc]init];
      NSArray *newArrayAttempt = [newTry arraySorter:numberArray3];
        ArraySorting *forNewArrayAttempt = [[ArraySorting alloc]init];
        NSArray *newArrayAttemptSorted = [forNewArrayAttempt arraysorter:newArrayAttempt];
        for (NSNumber *d in newArrayAttemptSorted) {
            NSLog(@"%@", d);  
        }
        long lastArrayIndex = newArrayAttemptSorted.count -1;
        
        NSLog(@"new final final answer is %@", newArrayAttemptSorted[lastArrayIndex]);
       ...
       NSArray *finalArray = [sortedArray arraysorter:finalProductArray];
        ...
        double finalNumber1 = [finalNumber doubleValue];
        
        NSLog(@"final number is %f", finalNumber1);
